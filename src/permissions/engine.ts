  ...
  async authorize(
    ...
    const audit = (outcome: AuditOutcome, reason?: string) =>
      writeAudit({
        ...
        companyId: operation.companyId ?? 'local',
    ...
    if (this.policy.readOnlyMode && operation.capability === 'write') {
      await audit('denied', 'HEALTH_MCP_MODE=readonly');
      throw new PermissionDeniedError(
        `${operation.name} is a write operation and the server runs in read-only mode (HEALTH_MCP_MODE=readonly).`,
      );
    }
    ...
    if (this.requiresConfirmation(operation.scope)) {
      if (options.confirmationToken) {
        ...
        const preview = operation.preview
          ...
          : `Execute ${operation.name}${operation.companyId ? ` on ${operation.companyId}` : ''}.`;
