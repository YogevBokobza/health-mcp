...
import { policyPath as policyPathFromConfig } from '../config/paths.js';
import { z } from 'zod';
...
export interface ResolvedPolicy {
  ...
  /**
   * Hard global override from HEALTH_MCP_MODE=readonly. No profile can escalate past it —
...
/**
 * Picks the active profile. HEALTH_MCP_PROFILE wins over the file's defaultProfile so a
...
export function resolvePolicy(file: PolicyFile = loadPolicyFile()): ResolvedPolicy {
  const profileName = process.env.HEALTH_MCP_PROFILE ?? file.defaultProfile;
  ...
  return {
    profileName,
    profile,
    readOnlyMode: process.env.HEALTH_MCP_MODE === 'readonly',
  };
}
